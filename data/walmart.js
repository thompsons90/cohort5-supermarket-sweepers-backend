...
getWalmartChickenData = async() =>{
    ...
    if ((await page.$('.sans-serif.ph1.pv2.w4.h4.lh-copy.border-box.br-100.b--solid.mh2-m.db.tc.no-underline.gray.bg-white.b--white-90')) !== null) {
        //If class is available, then the content of this class will be used to determine iterations
        pageLength = parseInt(await page.evaluate(el => el.innerText, await page.$('.sans-serif.ph1.pv2.w4.h4.lh-copy.border-box.br-100.b--solid.mh2-m.db.tc.no-underline.gray.bg-white.b--white-90')))
    } else {
        //Length is based off of number of elements when missing a certain class on the page
        pageLength = await page.$$eval(".sans-serif.ph1.pv2.w4.h4.lh-copy.border-box.br-100.b--solid.mh2-m.db.tc.no-underline.black.bg-white.b--white-90", divs => divs.length) + 1      
    }
    ...
    for(let i = 1; i < pageLength+1; i++){
        ...
        await items.map(item => {
            ...
            if(meatName.includes("leg") || meatName.includes("Leg") || meatName.includes("drum") || meatName.includes("Drum") || meatName.includes("Thigh") || meatName.includes("thigh")){
                meatOption = "thighs"
                // console.log(item)
...
getWalmartPorkData = async () =>{
    ...
    for(let i = 1; i < pageLength+1; i++){
        ...
        await items.map(item => {
            ...
            if((meatName.includes("bacon") || meatName.includes("Bacon")) && (!meatName.includes("bits") || !meatName.includes("Bits") || !meatName.includes("Brat") || !meatName.includes("brat") || !meatName.includes("patties") || !meatName.includes("patties"))){
                meatOption = "bacon"
            }
...
const getWalmartGroundBeefData = async () =>{
    ...
    ///Loop through all pages on search query
    for(let i = 1; i < pageLength+1; i++){
        await page.goto(`https://www.walmart.com/search?q=ground+beef&typeahead=beef&page=${i}&affinityOverride=store_led`, {
            waitUntil: 'networkidle2',
        });
        console.log(i)
